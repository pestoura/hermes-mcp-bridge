#!/usr/bin/env bash
# Read-only GO/NO-GO preflight for the 0.9.0 rollout.
...
EXPECTED_SHA_0_9_0="${EXPECTED_SHA_0_9_0:-}"
...
require_cmd docker df awk stat python3 curl jq

[ -n "$EXPECTED_SHA_0_9_0" ] || fail "EXPECTED_SHA_0_9_0 obrigatorio"
[ -n "$ROLLBACK_IMAGE" ] || fail "ROLLBACK_IMAGE obrigatorio"
[ -n "$ROLLBACK_IMAGE_ID" ] || fail "ROLLBACK_IMAGE_ID obrigatorio"

assert_image_revision "$CANDIDATE_IMAGE" "$EXPECTED_SHA_0_9_0"
...
if docker inspect "$CONTAINER_NAME" >/dev/null 2>&1; then
  ...
  [ "$current_image_id" = "$ROLLBACK_IMAGE_ID" ] \
    || fail "producao atual nao corresponde ao ID imutavel de rollback"
  ok "producao atual healthy e alinhada com o rollback imutavel"
else
...
require_file "$BRIDGE_STATE_DIR/state.sqlite3"
[ -d "$BRIDGE_POLICY_DIR" ] || fail "policy dir ausente"
[ -d "$BRIDGE_SECRETS_DIR" ] || fail "secrets dir ausente"

# Transitional rollback contract: 0.8.2 still needs the API key in the env file.
# Candidate 0.9.0 uses the mounted secret and therefore never consumes that
# value. Keep dual provisioning only until the rollback baseline supports files.
env_has_nonempty "$BRIDGE_ENV_FILE" "HERMES_API_KEY" \
  || fail "HERMES_API_KEY no env file e obrigatoria para rollback 0.8.2"
...
do
  if env_is_truthy "$BRIDGE_ENV_FILE" "$flag"; then
    fail "$flag deve permanecer desativado no rollout base 0.9.0"
  fi
done
ok "features opcionais permanecem desativadas"
