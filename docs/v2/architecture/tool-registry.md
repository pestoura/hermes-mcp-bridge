...
`canonical_name`, `version`, `input_schema`, `output_schema`, `read_only`, `mutation_class`, `risk_class`, `policy_action`, `required_resource_scope`, `credential_capability`, `idempotency_semantics`, `timeout`, `retry_class`, `concurrency_hint`, `lock_hint`, `backend`, `provenance`, `result_shaping`, `cost_hint`, `stability`, `deprecated`, `version_added`.

Free-text descriptions remain useful editorial metadata but are not part of the
Phase 1 canonical/audit-safe form. This applies to both tools and capability
descriptors: descriptions may exist in memory/source documentation but are
excluded from capability snapshots and client projections.
...
The Phase 1 implementation uses the explicit states `CONFIGURED`, `AVAILABLE`,
`HEALTHY`, `READY`, `DEGRADED`, `UNAVAILABLE`, `DENIED`; `DENIED` is the
implemented name for the ADR's `UNAUTHORIZED`. Only `READY` may execute.

## Capability snapshots

Each execution should be able to record `capability_snapshot_hash` so audit/replay can identify the exact tool surface and versions used.

Phase 1 defines that hash as SHA-256 over deterministic canonical JSON. The
snapshot includes only structured non-secret metadata; free-text tool and
capability descriptions are deliberately excluded.
...
- `ToolDefinition` (`schema.py`) — canonical typed definition carrying
  ...
  `result_shaping`, `capability_id`, `version`, plus structured non-secret
  `backend`/`stability`/`deprecated` metadata. Free-text `description` exists
  only as editorial in-memory metadata and is non-canonical/non-projected.
- `CapabilityDescriptor` (`capabilities.py`) — stable capability ID, provider,
  readiness state and version. Its free-text `description` is likewise
  non-canonical and excluded from the snapshot hash.
...
- `capability_snapshot_hash` — canonical JSON (UTF-8, sorted keys, separators
  ...
  `capability_id`, no timestamps, paths or editorial descriptions) digested
  with SHA-256 into a lowercase 64-hex string. Insertion order cannot change it;
  any material change (tool set, schema, state, version, tier, policy action,
  timeout) does.
...
snapshot version; dynamic discovery/refresh (OD-012); real credential backend
(OD-005); principal/tenant model (OD-007).
