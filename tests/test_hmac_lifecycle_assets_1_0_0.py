...
def test_hmac_runbook_pins_state_boundaries_and_cleanup() -> None:
    text = (ROOT / "docs" / "hmac-lifecycle-1.0.0.md").read_text(encoding="utf-8")
...
def test_hard_grace_limit_is_not_environment_configurable() -> None:
    source = (ROOT / "src" / "hermes_mcp_bridge" / "signing.py").read_text(encoding="utf-8")

    assert "MAX_PREVIOUS_GRACE_SECONDS = 7 * 24 * 60 * 60" in source
    assert "MAX_PREVIOUS_GRACE_SECONDS" not in (ROOT / ".env.example").read_text(encoding="utf-8")
